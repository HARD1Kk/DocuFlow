        ...
        with log_context(request_id=query_id, stage="Retrieval"):
            ...
            try:
                ...
                if not query_embeddings:
                    self.logger.warning("Empty query embedding generated", extra={"latency_ms": (time.perf_counter() - start_time) * 1000})
                    return []
                ...
                if not documents:
                    self.logger.warning(
                        f"Empty result set returned for query '{query}'",
                ...
                retrieved_chunks = []
                self.logger.info(
                    f"Retrieved {len(documents)} chunks (latency={latency_ms:.2f}ms)",
                    extra={"latency_ms": latency_ms}
                )
                ...
                for doc_id, doc, dist, meta in zip(ids, documents, distances, metadatas):
                    ...
                    score = 1.0 - dist
                    self.logger.info(
                        f"Retrieved chunk: id={doc_id}, score={score:.4f}",
                        extra={"latency_ms": latency_ms}
                    )
