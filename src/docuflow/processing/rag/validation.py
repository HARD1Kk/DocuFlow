...
class ValidationLayer:
    ...
    def validate(self, query: str, context: str, response: str) -> bool:
        ...
        with log_context(stage="Validation"):
            ...
            if not gatekeeper_passed:
                self.logger.warning(
                    f"Gatekeeper check failed: Response does not address the query '{query}'"
                )
                return False
    ...
    def _run_gatekeeper(self, query: str, response: str) -> bool:
        if self.llm_service:
            prompt = f"Does the following answer address the query? Query: {query} Answer: {response}. Answer yes or no."
            try:
