    ...
    def query(self, query: str, top_k: int = 5) -> str:
        ...
        with log_context(request_id=query_id, stage="Context Assembly"):
            ...
            for i, chunk in enumerate(retrieved_chunks):
                ...
                if context_tokens + chunk_tokens <= self.max_context_tokens:
                    ...
                    self.logger.info(
                        f"Chunk index {i} accepted: tokens={chunk_tokens} (running_total={context_tokens})"
                    )
                else:
        ...
        with log_context(request_id=query_id, stage="Generation"):
            ...
            if not self.llm_service:
                ...
                try:
                    ...
                    gen_latency = (time.perf_counter() - gen_start) * 1000
                    self.logger.error(f"LLM generation failed: {e}", exc_info=True, extra={"latency_ms": gen_latency})
                    raise
        ...
        if self.validator:
            with log_context(request_id=query_id, stage="Validation"):
                ...
                if not is_valid:
                    self.logger.warning(
                        "RAG answer validation failed. Triggering fallback response rejection.", extra={"latency_ms": 0}
                    )
